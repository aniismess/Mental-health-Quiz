...
export default function AdminDashboard() {
  ...
  try {
    ...
    const { data: usersData, error: usersError } = await supabase
      .from("users")
      .select(`*, quiz_sessions!inner(all_completed, id, vak_completed, ei_completed, rep_system_completed, completed_at)`)
      .order("created_at", { ascending: false })

    if (usersError) throw usersError

    const processedUsersForExport = usersData.map((user: any) => ({
      ...user,
      vak_completed: user.quiz_sessions?.[0]?.vak_completed || false,
      ei_completed: user.quiz_sessions?.[0]?.ei_completed || false,
      rep_system_completed: user.quiz_sessions?.[0]?.rep_system_completed || false,
    ...
    for (const user of usersData) {
      ...
      if (vakRespResult.error) throw vakRespResult.error
      if (vakResResult.error) throw vakResResult.error
      if (eiRespResult.error) throw eiRespResult.error
      if (eiResResult.error) throw eiResResult.error
      if (repRespResult.error) throw repRespResult.error
      if (repResResult.error) throw repResResult.error

      // VAK responses
      ;(vakRespResult.data || []).forEach((resp: any) => {
        exportData.push({
          user_id: user.id,
          user_name: user.name,
          ...
          vak_question_number: resp.question_number,
          vak_question_text: resp.question_text,
          vak_selected_option: resp.selected_option,
          vak_selected_value: resp.selected_value,
          vak_visual_score: vakResResult.data?.[0]?.visual_score ?? "",
          vak_auditory_score: vakResResult.data?.[0]?.auditory_score ?? "",
          vak_kinesthetic_score: vakResResult.data?.[0]?.kinesthetic_score ?? "",
          vak_dominant_style: vakResResult.data?.[0]?.dominant_style ?? "",
        })
      })

      // EI responses
      ;(eiRespResult.data || []).forEach((resp: any) => {
        exportData.push({
          user_id: user.id,
          user_name: user.name,
          user_email: user.email,
          user_course: user.course_name,
          user_semester: user.semester,
          ei_statement_number: resp.question_number,
          ei_statement_text: resp.question_text,
          ei_rating: resp.response_value,
          ei_category: resp.category,
          ei_self_awareness_score: eiResResult.data?.[0]?.self_awareness_score ?? "",
          ei_managing_emotions_score: eiResResult.data?.[0]?.managing_emotions_score ?? "",
          ei_motivating_oneself_score: eiResResult.data?.[0]?.motivating_oneself_score ?? "",
          ei_empathy_score: eiResResult.data?.[0]?.empathy_score ?? "",
          ei_social_skill_score: eiResResult.data?.[0]?.social_skill_score ?? "",
          ei_total_score: eiResResult.data?.[0]?.total_score ?? "",
        })
      })

      // Rep System responses
      ;(repRespResult.data || []).forEach((resp: any) => {
        const options = [
          { text: "Visual", value: "V", rank: resp.visual_rank },
          { text: "Auditory", value: "A", rank: resp.auditory_rank },
          { text: "Kinesthetic", value: "K", rank: resp.kinesthetic_rank },
          { text: "Auditory Digital", value: "Ad", rank: resp.auditory_digital_rank },
        ]
        options.forEach((opt, idx) => {
          exportData.push({
            ...
            user_course: user.course_name,
            user_semester: user.semester,
            rep_question_number: resp.question_number,
            rep_option_number: idx + 1,
            rep_ranking: opt.rank,
            rep_option_text: opt.text,
            rep_option_value: opt.value,
            rep_visual_score: repResResult.data?.[0]?.visual_total ?? "",
            rep_auditory_score: repResResult.data?.[0]?.auditory_total ?? "",
            rep_kinesthetic_score: repResResult.data?.[0]?.kinesthetic_total ?? "",
            rep_auditory_digital_score: repResResult.data?.[0]?.auditory_digital_total ?? "",
            rep_dominant_system: repResResult.data?.[0]?.dominant_system ?? "",
          })
        })
      })
    }

    // ✅ now generate CSV just like before
    let csvContent = ""
    let filename = ""
    ...
    switch (exportType) {
      case "summary":
        csvContent = generateUserSummaryCSV(processedUsersForExport)
    ...
    toast({ title: "Export Successful", description: "Quiz data exported successfully." })
                ...
                <SelectContent>
                  ...
                  {getUniqueValues("semester").map((semester) =>
                    semester !== null && semester !== false ? (
                      <SelectItem key={String(semester)} value={semester.toString()}>
                        Semester {semester}
                      </SelectItem>
                    ) : null
                  )}
